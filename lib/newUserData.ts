...
export async function fetchCustomerSuspensionByEmail(email: string): Promise<CustomerSuspension | null> {
  if (!isSupabaseConfigured || !supabase) return null;
  ...
  if (error) {
    const missingSuspensionColumns = /column\s+"(suspended|suspension_reason)"\s+does not exist/i.test(error.message ?? "");
    if (missingSuspensionColumns) return null;
    throw error;
  }
...
export async function requestAccountGeneration(email: string) {
  ...
  const { error } = await supabase.from("customers").update({
    review_request: "account_generation",
    review_requested_at: new Date().toISOString(),
    account_details: null,
    account_details_sent_at: null,
    crypto_name: null,
...
export async function notifyAdminMoneySent(email: string) {
  if (!isSupabaseConfigured || !supabase) throw new Error("Supabase is not configured.");

  const { error } = await supabase.from("customers").update({
    review_request: "money_sent",
    review_requested_at: new Date().toISOString(),
  }).eq("email", email);
...
export async function sendAccountDetails(email: string, accountDetails: string) {
  if (!isSupabaseConfigured || !supabase) throw new Error("Supabase is not configured.");
  const details = accountDetails.trim();
  if (!details) throw new Error("Account details are required.");
...
export async function updateCustomerSuspension(email: string, suspended: boolean, suspensionReason?: string) {
  ...
  const { data, error } = await supabase
    ...
    .update({
      suspended,
      suspension_reason: suspended ? suspensionReason?.trim() || "Your account has been suspended." : null,
    })
    .eq("email", email)
...
export async function resetAccountDetails(email: string) {
  if (!isSupabaseConfigured || !supabase) {
    throw new Error("Supabase is not configured.");
  }
  ...
  return {
    ...
    suspended: customerRecord.suspended === true,
    suspensionReason: customerRecord.suspension_reason ? String(customerRecord.suspension_reason) : undefined,
    reviewRequest: "account_generation",
...
export async function fetchRegisteredNewUsers(): Promise<CustomerSuspension[]> {
  if (isSupabaseConfigured && supabase) {
    const { data, error } = await supabase
      .from("customers")
      .select("full_name, email, phone, account_number, suspended, suspension_reason, review_request, review_requested_at, account_details, account_details_sent_at, crypto_name, crypto_address, crypto_payment_time")
    ...
    if (!error && Array.isArray(data) && data.length > 0) {
      return data
        .map((item) => {
          const customerRecord = item as unknown as Record<string, unknown>;
          return {
            email: String(customerRecord.email ?? ""),
            fullName: String(customerRecord.full_name ?? "New Customer"),
            phone: customerRecord.phone ? String(customerRecord.phone) : undefined,
  ...
  return getRegisteredNewUsers().map((user) => ({ ...user, suspended: false, suspensionReason: undefined, reviewRequest: undefined, reviewRequestedAt: undefined, accountDetails: undefined, accountDetailsSentAt: undefined }));
}
