...
import { getNewUserSession, loadNewUserReceipts, loadNewUserTransfers, type NewUserReceipt } from "@/lib/newUserData";
  ...
  useEffect(() => {
    ...
    async function loadReceipt() {
      ...
      if (isMounted) {
        ...
        setIsPending(transfers.some((item) => item.reference === reference && item.status === "Pending"));
      }
    }
          ...
          <div className="transactions-shell">
            <div className="transactions-header">
              <div>
                ...
                <h1>{isPending ? "Receipt pending" : "Receipt not found"}</h1>
                <p>{isPending ? "Your receipt will be generated when your transfer is approved." : "No receipt exists for this transaction reference."}</p>
              </div>
            </div>
          </div>
        </section>
